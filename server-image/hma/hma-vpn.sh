...
$curl http://securenetconnection.com/vpnconfig/servers-cli.php 2>/dev/null| grep -i -e "$grep" | grep -i -e "$proto" > /tmp/hma-servers
...
$curl "http://securenetconnection.com/vpnconfig/openvpn-template.ovpn" > /tmp/hma-config.cfg 2>/dev/null

# BEGIN DEVIATION FROM OFFICIAL
echo "auth-user-pass /tmp/login.conf" >> /tmp/hma-config.cfg
# END DEVIATION FROM OFFICIAL
